        ...
        if(in_array($fileActualExt, $allowed))
        {
            if($fileError === 0)
            {
                if($fileSize < 1000000)
                {
                    ...
                    header("Location: phonedetails.php?productid={$row["product_id"]}");
                }
                else
                {
                    echo "File too large";
...
<head>
    ...
    <title>Upload Phone Image</title>
...
<body>
    ...
    <main>
        <div><h1>Upload Phone Image</h1></div>
        <div class="LoginForm">
            <div class="FormMargin">
                <h2>Image</h2>
                <form action="uploadphone.php?product_id=<?php echo $productid;?>" method="post" enctype="multipart/form-data">
                    <label for="product_image">Image Upload: </label><br>
                        <input type="file" id="product_image" name="product_image"><br>
                    <input type="submit" value="Upload" name="phoneupload">
                </form>
            </div>
        </div>
    </main>
